...
def Macula_subservices_page(request):
    if request.method == 'GET':
        ...
        with open(f"{pathout}/out.txt", "w") as out:
            classifier = keras.models.load_model(f'F:/GraduationProject/oct/ML/model2/MaculaClassifier.h5')
            ...
            if np.argmax(classifier.predict_on_batch(im)) == 0:
                ...
                model = load_model(f'F:/GraduationProject/oct/ML/model2/my_model.h5')
